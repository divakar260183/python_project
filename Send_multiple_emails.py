...
def send_email(counter):
    ...
    msg['Subject'] = "Mail After Password Change " + ' ' + str(counter)
    msg.set_content("Mail After Password Change " + ' ' + str(counter))
    print(" Message Sent to : PattiF@7wn1tk.onmicrosoft.com")
    s.send_message(msg)
    s.quit()

def send_email_at(send_time, counter):
    time_now = dt.datetime.utcnow()
    time_now_timestamp = dt.datetime.timestamp(time_now)
    time.sleep(send_time.timestamp() - time_now_timestamp)
    send_email(counter)
    print(str(counter) + ' email sent')
